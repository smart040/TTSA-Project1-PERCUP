...
plt.plot(t, funct(t))
#plt.show()
plt.savefig('./signals_50/'+str(power)+"_"+str(base)+"_"+"SignalsPics.pdf", dpi=150)
...
plt.savefig('./pre_dgms_50/'+str(power)+"_"+str(base)+"_"+"Pre-pdgmspdf", dpi=150)
#plt.show()
...
plt.savefig('./pre_dgms_50/'+str(power)+"_"+str(base)+"_"+"Pre-pdgmsaver", dpi=150)
#plt.show()
...
def QPDS(dgm): #This one is used for all 
...
#plt.show()
    ...
    dgm_w_cup = plt.savefig('./post_dgms_50/'+str(power)+"_"+ str(base)+"_"+str(Av_QPDS)+"_PDiagrams.pdf", dpi=150)
    #plt.show()
...
if should_run_QPDS(dgm1) == False:
    print(should_run_QPDS(dgm1))
    Av_QPDS = 0 
    print("Quasiperiodicity Score: 0")
   
else:
    Av_QPDS = QPDS(diagrams)[0]
    print (Av_QPDS) 
...
df1 = pd.DataFrame({'power' : [power], 
               		'base': [base],
			'Should Run': should_run_QPDS(dgm1),
                    	'QPDS': [Av_QPDS]})

df1.to_csv('./tables_50/'+str(power)+"_"+ str(base)+'_power_base_QPDS_.csv')
